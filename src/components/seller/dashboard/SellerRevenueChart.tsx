...
const PADDING = { top: 18, right: 24, bottom: 42, left: 104 };
...
export function SellerRevenueChart({
  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
      <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
        ...
        <div className="flex flex-wrap items-end gap-2">
          ...
          <button
            ...
            className="hidden size-10 place-items-center rounded-xl border border-slate-200 text-slate-600 transition hover:border-violet-200 hover:bg-violet-50 hover:text-violet-700 disabled:cursor-not-allowed disabled:opacity-40 sm:grid"
            aria-label="Xem tháng trước"
          >
            <ChevronLeft className="size-4" />
          </button>
          <label className="min-w-44 flex-1 text-[10px] font-bold uppercase tracking-wide text-slate-500 sm:flex-none">
            Chọn tháng
            <span className="relative mt-1 block">
              ...
              <input
                ...
                disabled={isLoading}
                className="h-10 w-full rounded-xl border border-slate-200 bg-white pl-10 pr-3 text-sm font-bold text-slate-700 outline-none transition focus:border-violet-400 focus:ring-4 focus:ring-violet-500/10 disabled:cursor-wait disabled:bg-slate-50"
              />
            </span>
          </label>
          <button
      ...
      <div className="mt-4 overflow-x-auto rounded-xl border border-slate-100 bg-slate-50/60 p-3 sm:p-4">
