...
const STATUS_STYLE:Record<string,string>={ACTIVE:"bg-emerald-50 text-emerald-700",SUCCESS:"bg-emerald-50 text-emerald-700",DONE:"bg-emerald-50 text-emerald-700",COLLECTED:"bg-emerald-50 text-emerald-700",RESOLVED:"bg-emerald-50 text-emerald-700",APPROVED:"bg-sky-50 text-sky-700",PENDING:"bg-amber-50 text-amber-700",OPEN:"bg-amber-50 text-amber-700",REVIEW_REQUIRED:"bg-orange-50 text-orange-700",SUSPENDED:"bg-orange-50 text-orange-700",BANNED:"bg-rose-50 text-rose-700",REJECTED:"bg-rose-50 text-rose-700",FAILED:"bg-rose-50 text-rose-700",INACTIVE:"bg-slate-100 text-slate-600",EXPIRED:"bg-slate-100 text-slate-600"};
const STATUS_LABEL:Record<string,string>={PENDING:"Chờ kiểm tra",APPROVED:"Đang chuyển khoản",DONE:"Đã chuyển tiền",REJECTED:"Đã từ chối",WITHDRAW_PENDING:"Gửi yêu cầu rút",WITHDRAW_DONE:"Đã rút tiền",WITHDRAW_CANCEL:"Hoàn yêu cầu rút"};
export function AdminStatus({value}:{value:string}) { return <span className={`inline-flex rounded-full px-2.5 py-1 text-[11px] font-black ${STATUS_STYLE[value]||"bg-violet-50 text-violet-700"}`}>{STATUS_LABEL[value]||value.replaceAll("_"," ")}</span>; }
